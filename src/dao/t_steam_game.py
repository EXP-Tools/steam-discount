...
class TSteamGameDao(BaseDao) :
    ...
    def _to_bean(self, row) :
        ...
        if row:
            ...
            bean.game_id = self._to_val(row, 1)
            bean.game_name = self._to_val(row, 2)
            bean.original_price = self._to_val(row, 3)
            bean.lowest_price = self._to_val(row, 4)
            bean.discount_rate = self._to_val(row, 5)
            bean.discount_price = self._to_val(row, 6)
            bean.evaluation_id = self._to_val(row, 7)
